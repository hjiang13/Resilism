...
CJmpMap jmp_map;
VOID printip(VOID * ip1) { 
	uintptr_t i; 
	i = reinterpret_cast<uintptr_t>(ip1);
	if (i < 0x80000000)
		printf("%p\n", ip1); 
	}
VOID FI_InjectFault_FlagReg(VOID * ip, UINT32 reg_num, UINT32 jmp_num, CONTEXT* ctxt)
{
	//printf("I am now in FI flag register function \n");
	if(fi_iterator == fi_inject_instance) {

	//	printf("fi_iterator is equal to fi_inject_instance \n");
	    	bool isvalid = false;
...
//					(VOID*)PIN_GetContextReg( ctxt, reg )));
		printf("EXECUTING flag reg: Original Reg name %s value %p\n", REG_StringShort(reg).c_str(),(VOID*)PIN_GetContextReg( ctxt, reg ));
...
//					(VOID*)PIN_GetContextReg( ctxt, reg )));
			printf("EXECUTING flag reg: Changed Reg name %s value %p\n", REG_StringShort(reg).c_str(),(VOID*)PIN_GetContextReg( ctxt, reg ));			
...
VOID FI_InjectFault_Mem(VOID * ip, VOID *memp, UINT32 size)
{
		if(fi_iterator == fi_inject_instance) {
			if(size == 4) {
				PRINT_MESSAGE(4, ("Executing %p, memory %p, value %d, \n", ip, memp, * ((int*)memp)));
			}
...
{
	//files that stores the instruction count
	// here in Hailong Jiang's Project is 'pin.instcount.txt'
	...
	//fi_inject_instance =1;//Here we set this fi_inject_instance to 1 to check if it always inject fault into the same iteration.
	//PRINT_MESSAGE(4, ("Instance:%llu\n",fi_inject_instance));
	fclose(fi_input_FILE);	
}
//Hailong Jiang 
//Trace all the instruction in execution
VOID Trace(TRACE trace, VOID *v){
	for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)){
		BBL_InsertCall(bbl, IPOINT_BEFORE, (AFUNPTR)printip, IARG_ADDRINT,BBL_Address(bbl), IARG_END);
	}
}

//VOID printip(VOID *ip) { fprintf("%p\n", ip); }
