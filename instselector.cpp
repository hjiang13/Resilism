...
static string datafile = "pin.data.instselector.txt";
...
bool isInstFITarget(INS ins ) {
  bool ret = false;
  string strIP;
  //load data from pin.data.instselector.txt
  std::ifstream file("/home/veloc/pin-v3.5/source/tools/pinfi/pin.data.instselector.txt");
  if (file.good())
  {
    std::getline(file,strIP);
  
  //  std::cerr<< strIP<<endl;
  }
 
  std::stringstream hexIP;
  hexIP << std::hex<<strIP;
  ADDRINT targetInsIP;
  hexIP >> targetInsIP;
  //targetInsIP = std::atoi(strIP.c_str());
    //debug
  //targetInsIP = 0x400552;
  //std::cerr << hex << "The target Ins Ip is " << targetInsIP << endl;
...
/*  // check include
  ...
  if (INS_Address(ins)== targetInsIP)
	  ret = true;
